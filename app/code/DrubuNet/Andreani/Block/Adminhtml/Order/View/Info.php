...
namespace DrubuNet\Andreani\Block\Adminhtml\Order\View;
...
class Info extends \Magento\Sales\Block\Adminhtml\Order\View\Info
{
    ...
    public function getAndreaniOriginAddress(){
        ...
        $originAddress = $andreaniOrder->getOriginAddress();
        ...
        foreach ($originAddress->getData() as $field => $value){
    ...
    public function getAndreaniDestinationAddress(){
         ...
         */
        $andreaniOrder = $this->_coreRegistry->registry('current_andreani_order');
        $destinationAddress = $andreaniOrder->getDestinationAddress();

        $html = "%customer_firstname %customer_lastname (DNI: %customer_vat_id)<br>%street %number, %floor %apartment<br>%city, %postcode, %region<br>Telefono: %customer_telephone<br>Observaciones: %observations";

        foreach ($destinationAddress->getData() as $field => $value){
            $html = str_replace('%'.$field, $value, $html);
        }
